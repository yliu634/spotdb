...
bool  SpotkvDB::hasRead = false;
int SpotkvDB::Read(const string& table, const string& key, const vector< string >* fields, vector< DB::KVPair >& result)
{
    std::string value;
...
int SpotkvDB::Insert(const string& table, const string& key, vector< DB::KVPair >& values)
{
    leveldb::Status s;
    int count = 0;
    for(KVPair &p : values){
...
int SpotkvDB::Scan(const string& table, const string& key, int len, const vector< string >* fields, vector< vector< DB::KVPair > >& result)
{
...
void SpotkvDB::printFilterCount()
{
    int fd[7],i;
    int levels = 0;
    char buf[100];
